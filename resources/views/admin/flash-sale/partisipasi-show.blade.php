<x-layouts.admin>
    ...
    @php
        ...
        $jumlahDisertakan = $flashSale->produks->filter(fn ($b) => $b->produk?->toko_id === $toko->id)->count();
    @endphp
                ...
                @unless ($terkunci)
                    <form method="POST" action="{{ route('admin.flash-sale.ikut', $flashSale) }}">
                        ...
                        <button class="inline-flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-bold transition
                            ...
                            {{ $diikuti ? 'Berhenti Ikut' : 'Ikuti Kampanye' }}
                        </button>
                    </form>
                @endunless
            </div>
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 p-6">
            <div>
                ...
                <p class="mt-0.5 text-xs text-slate-400">
                    Harga flash sudah terisi otomatis −{{ $flashSale->label_diskon }} dari harga normal.
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($produks as $item)
                        @php
                            ...
                            $usulan = (int) $flashSale->hargaSetelahPotongan((float) $produk->harga);
