                    ...
                    @foreach ($items as $item)
                        <div class="card flex flex-col gap-4 p-5 sm:flex-row sm:items-center">
                            <a href="{{ route('toko.show', $item->produk->slug) }}" class="shrink-0 overflow-hidden rounded-2xl bg-slate-100">
                                ...
                                @else
                                    <div class="flex h-24 w-24 items-center justify-center text-4xl sm:h-28 sm:w-28">🛍️</div>
                                @endif
                            </a>
                            <div class="min-w-0 flex-1">
                                <a href="{{ route('toko.show', $item->produk->slug) }}" class="line-clamp-2 text-sm font-bold text-slate-800 hover:text-indigo-700">{{ $item->produk->nama }}</a>
                                <p class="mt-1 text-xs font-medium text-slate-400">{{ $item->produk->kategori?->nama }}</p>
                                <p class="mt-2 text-sm font-extrabold text-indigo-700">{{ rp($item->produk->harga) }}</p>
                            </div>
                            <div class="flex items-center justify-between gap-4 sm:flex-col sm:items-end">
                                <form action="{{ route('keranjang.updateQty', $item) }}" method="POST">
                                    ...
                                    <div class="flex items-center rounded-xl ring-1 ring-slate-300">
                                        <button type="button" onclick="let i=this.parentElement.querySelector('input'); i.value=Math.max(1, +i.value-1); this.closest('form').submit();" class="px-3 py-2 text-sm font-bold text-slate-500 hover:text-indigo-600">−</button>
                                        ...
                                        <button type="button" onclick="let i=this.parentElement.querySelector('input'); i.value=Math.min(+i.max||999, +i.value+1); this.closest('form').submit();" class="px-3 py-2 text-sm font-bold text-slate-500 hover:text-indigo-600">+</button>
                                    </div>
                                </form>
                                <div class="flex items-center gap-3">
                                    ...
                                    <form action="{{ route('keranjang.hapus', $item) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button class="text-lg text-rose-400 transition hover:text-rose-600" title="Hapus">🗑️</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                ...
                <div class="lg:sticky lg:top-24 lg:self-start">
                    <div class="card p-6">
                        ...
                        <div class="mt-5 border-t border-dashed border-slate-200 pt-4">
                            ...
                            <a href="{{ route('toko.index') }}" class="mt-3 block text-center text-sm font-bold text-indigo-600 hover:text-indigo-800">← Lanjut Belanja</a>
                        </div>
                    </div>
                </div>
            </div>
