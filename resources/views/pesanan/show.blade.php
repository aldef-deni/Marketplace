    ...
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('pesanan.cetak', $pesanan->no_invoice) }}" target="_blank" class="btn-secondary btn-sm">🖨️ Cetak</a>
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            ...
            <div class="space-y-6 lg:col-span-2">
                            ...
                            @foreach ([
                                ...
                                <div class="flex flex-1 flex-col items-center {{ $i === 3 ? '' : 'sm:flex-row' }} {{ $i > 0 ? 'ml-2 sm:ml-0' : '' }}">
                                    ...
                                    <div class="flex flex-col items-center">
                                        <span class="flex h-10 w-10 items-center justify-center rounded-full text-sm font-extrabold ring-4 ring-white {{ $pesanan->status === 'dibatalkan' ? 'bg-rose-500 text-white' : ($langkah > $i ? 'bg-emerald-500 text-white' : ($langkah === $i && ! in_array($pesanan->status, ['menunggu_pembayaran', 'menunggu_konfirmasi', 'diproses', 'dikirim', 'selesai']) ? 'bg-slate-300 text-white' : ($langkah >= $i ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-400'))) }}">
                                            {{ $langkah > $i ? '✓' : $angka }}
                                        </span>
                        ...
                        <div class="p-6">
                            <div class="flex items-center gap-4 rounded-2xl bg-slate-50 p-5 ring-1 ring-slate-200/70">
                                <span class="flex h-14 w-14 items-center justify-center rounded-2xl text-3xl {{ match ($pesanan->pembayaran->metodePembayaran->tipe) { 'transfer' => 'bg-blue-100', 'ewallet' => 'bg-emerald-100', 'cod' => 'bg-amber-100', default => 'bg-slate-100' } }}">
                                    {{ match ($pesanan->pembayaran->metodePembayaran->tipe) { 'transfer' => '🏦', 'ewallet' => '📱', 'cod' => '💵', default => '💳' } }}
                                </span>
                                <div class="flex-1">
                            ...
                            @if ($pesanan->pembayaran->metodePembayaran->instruksi)
                                <div class="mt-4 rounded-2xl bg-amber-50/70 p-4 text-sm leading-relaxed text-amber-800 ring-1 ring-amber-100">
                                    💡 <span class="font-bold">Instruksi:</span> {{ $pesanan->pembayaran->metodePembayaran->instruksi }}
                                </div>
                            @endif

                            {{-- Form unggah bukti --}}
                            <form action="{{ route('pesanan.bayar', $pesanan) }}" method="POST" enctype="multipart/form-data"
                                  class="mt-5 rounded-2xl border-2 border-dashed border-indigo-200 bg-indigo-50/40 p-5"
                                ...
                                <div class="mt-4 grid gap-3 sm:grid-cols-2">
                                    <div>
                                        <input type="file" name="bukti" accept="image/*" required
                                               class="block w-full cursor-pointer rounded-xl border border-slate-300 bg-white p-2.5 text-xs text-slate-600 file:mr-3 file:rounded-lg file:border-0 file:bg-indigo-600 file:px-4 file:py-2 file:text-xs file:font-bold file:text-white hover:file:bg-indigo-700">
                ...
                @if ($pesanan->status === 'menunggu_konfirmasi')
                    <section class="card p-6">
                        <div class="flex items-start gap-4">
                            <span class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-orange-100 text-2xl">🔍</span>
                            <div>
                ...
                @if (in_array($pesanan->status, ['diproses', 'dikirim', 'selesai']))
                    <section class="card p-6">
                        <div class="flex items-start gap-4">
                            <span class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl text-2xl {{ $pesanan->status === 'selesai' ? 'bg-emerald-100' : 'bg-indigo-100' }}">
                                {{ match ($pesanan->status) { 'diproses' => '📦', 'dikirim' => '🚚', 'selesai' => '✅', default => '📦' } }}
                            </span>
                            <div class="flex-1">
                ...
                @if ($pesanan->status === 'dibatalkan')
                    <section class="card p-6">
                        <div class="flex items-start gap-4">
                            <span class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-rose-100 text-2xl">❌</span>
                            <div>
                ...
                <section class="card p-6">
                        ...
                        @foreach ($pesanan->items as $item)
                            <div class="flex items-center gap-4">
                                <div class="relative shrink-0">
                                    ...
                                    @else
                                        <div class="flex h-16 w-16 items-center justify-center rounded-xl bg-slate-100 text-2xl">🛍️</div>
                                    @endif
                    ...
                    <dl class="mt-5 space-y-2.5 border-t border-dashed border-slate-200 pt-5 text-sm">
                        ...
                        <div class="flex justify-between border-t border-slate-100 pt-3 text-base"><dt class="font-extrabold text-slate-900">Total</dt><dd class="font-extrabold text-indigo-700">{{ rp($pesanan->total) }}</dd></div>
                    </dl>
                </section>
            </div>
            ...
            <div class="space-y-6 lg:sticky lg:top-24 lg:self-start">
                <section class="card p-6">
                    ...
                    <div class="mt-3 flex items-center gap-2">
                        <span class="badge bg-indigo-50 text-indigo-700 ring-indigo-200">{{ $pesanan->alamat->label }}</span>
                    </div>
